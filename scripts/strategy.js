...
function classify(c) {
  ...
  const flags = [];
  if (c.liqRatio != null && c.liqRatio < 0.005) flags.push('very_low_liquidity');
  if (c.mcap != null && c.mcap < 5e7) flags.push('micro_cap');
  if (flags.length) return { group: 'high-risk', why: flags.join(', ') };
  if (['BTC', 'ETH'].includes(c.sym)) return { group: 'core', why: 'BTC/ETH — core reserve assets' };
  const liquid = c.liqRatio != null && c.liqRatio >= 0.01;
  const mature = c.samples != null && c.samples >= 330;
  if (!liquid) return { group: 'high-risk', why: `illiquid (vol/mcap ${round((c.liqRatio || 0) * 100, 2)}%)` };
  if (mature && c.mcap >= 3e9) return { group: 'established', why: `mature (${c.samples} daily candles ≥1y) + mcap $${round(c.mcap / 1e9, 1)}B ≥ $3B + liquid` };
  if (c.mcap >= 3e8 && (c.mcap < 3e9 || !mature)) return { group: 'emerging', why: `mcap $${round(c.mcap / 1e9, 2)}B ${!mature ? '(< ~1y history: ' + c.samples + ' candles)' : 'in $0.3–3B band'} + liquid` };
...
function backtest(closes) {
  ...
  for (let i = 200; i < closes.length; i++) {
    ...
    if (!inPos) { if (s50 > s200 && closes[i - 1] <= s20p && p > s20) { inPos = true; entry = p; } }
    else { if (p < s50) { trades.push(p / entry - 1); inPos = false; } }
  }
